...
- Gameplay logic has been extracted into `public/engine.js`.
- MVP event seed has been expanded to 45 cards.
...
- `npm run smoke` runs a Playwright mobile smoke test against a running local server, exercises the main loop, verifies all 8 low/high resource endings, and writes screenshots to `tmp/smoke/`.
- Sprint 1/2 review and QA notes are recorded in `docs/agentic-coding/sprint-1-review.md`.
- `npm run simulate -- 200 1000` found no hard max year under current rules; long-seeking strategy reached the 1000-year cap.
- Added rules data and `old_age_succession`; `npm run simulate -- 200 1000` now caps tested strategies at 60 years.
- Seven missing ending images were identified, but image generation is blocked by an invalid local OpenAI API key.

## TODO

- Inspect latest smoke screenshots after every meaningful UI change.
- Tune the 60-year old-age/succession cap after manual playtests.
- Add share-card generation and more polished result variants.
