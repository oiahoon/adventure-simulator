...
## 2026-03-02 人格画像系统 v1（行为证据）
- 新增结局层“行为人格画像”（非临床诊断）：
  - 维度：风险校准、社交驱动、压力恢复、规划视角
  - 输入：事件标签 + 属性变化 + 关键旗标 + 历史决策链
  - 输出：画像标题、各维度分值、置信度、Top 证据、画像建议
- 实现位置：`app/main.js`
  - 新增权重模型与标准化函数
  - `finishSession` 注入 `result.personality`
  - 分享文案增加“行为画像”摘要
- 结局 UI：`ui/game-ui.js`
  - 新增“行为人格画像”区块（含证据与建议）
- 文档：新增 `docs/product/personality-profiling-plan.md`
  - 给出题库映射基线（Big Five/IPIP、Mini-IPIP、SJT）与分阶段开发计划
- 版本缓存：`main.js?v=20260302_37`

## 2026-03-02 人格画像 UI 升级（JOJO风五边形）
- 结局“行为人格画像”模块新增：
  - 角色头像展示（与当前局角色头像一致）
  - 五边形雷达图（5 维）：风险校准 / 社交驱动 / 压力恢复 / 规划视角 / 执行推进
  - 雷达图标签带数值（0-100映射）
- 实现方式：`ui/game-ui.js` 内联 SVG 生成，`public/index.html` 增加雷达图样式。
- 画像模型同步升级为 5 维（`app/main.js`），并补充执行推进维度权重与建议逻辑。
- 资源缓存版本更新：`main.js?v=20260302_38`。
- 验证：语法检查通过，`npm test` 20/20 通过。

## 2026-03-02 人格画像校准增强（v1.1）
- 细粒度信号标签：新增 `signals` 机制并自动遍历全事件池注入（Opening/Chapter/Incident/Forced/Growth/Milestone/Wang线）。
- 题库映射升级：画像计算在 `tag + effect` 之外，叠加 `signals -> 维度向量`，减少“同 tag 不同语义”误差。
...
## UIUX Pass: 事件主导 + JOJO化视觉统一（2026-03-02）
- 标题体系更新：
  - 页面 title 改为「都市生活挑战」。
  - 顶部主标题改为「都市生活挑战」。
- 布局重构（突出主剧情事件）：
  - 删除独立大模块「补给商店/临时技能」卡片。
  - 将补给与技能并入状态卡底部，做成紧凑操作区（弱化存在感但保持可点击）。
  - 事件卡维持为主视觉核心，选项继续承担剧情推进。
- 装饰元素与组件融合改造：
  - 强化组件内纹理、角标、描边与投影，减少“图片贴在旁边”的割裂感。
  - 事件卡和选项卡统一纹理与内描边语言，按钮 hover/press 动势统一。
- JOJO风方向增强：
  - 全局背景增加低强度速度线纹理。
  - 章节标签改为斜切样式（skew）并保留像素角标。
  - 事件标题字重与对比加强，形成更强的视觉戏剧性。
- 缓存版本更新：`main.js?v=20260302_44`。
- 验证：`npm test` 21/21 通过。
