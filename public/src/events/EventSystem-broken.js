...
返回JSON格式：
{
  ...
  "impact_description": "影响描述"
    ...
    applyEventEffects(effects, gameState, impactDescription) {
        ...
        // 记录效果到UI
        if (hasEffects && window.gameEngine && window.gameEngine.uiManager && window.gameEngine.uiManager.addEffectsLog) {
            try {
                window.gameEngine.uiManager.addEffectsLog(effects, character);
            } catch (error) {
                console.warn('记录效果日志失败:', error);
            }
        }
        
        console.log('✅ 事件效果应用完成');
        return hasEffects;
    }
